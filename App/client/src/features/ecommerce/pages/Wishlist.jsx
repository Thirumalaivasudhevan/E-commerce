...
export default function Wishlist() {
  ...
  const removeItem = async (id) => {
    try {
      ...
      addToast('Success', 'Item removed from wishlist.', 'success');
    } catch (err) {
      console.error(err);
      addToast('Error', 'Failed to remove item from wishlist.', 'destructive');
    }
  };
          ...
          {wishItems.map((item) => (
            <div key={item.id} className="group bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300">
              <div className="aspect-[4/3] relative overflow-hidden">
                <img
                  src={item.product.image || `https://picsum.photos/seed/${item.product.id}/400/300`}
